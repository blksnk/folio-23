...
const itemPositionMobile = (x: number, y: number, isSelected = true, isMobile: boolean, b?: Breakpoint) => ({
  x: `calc(${cssCellWidth(x, true, b)} + ${cssUnit(!isMobile ? isSelected ? 0 : 1 : isSelected ? 1 : 2)})`,
  y: `calc(${cssCellHeightMobile(y, true, b)} + ${cssUnit(isSelected && !isMobile ? 0 : 1, b)})`
})
...
export const ArchiveGridItem = ({ item, select, isSelected, doPreview, selectedIndex, index, breakpoint, hide }: ArchiveGridItemProps) => {
  ...
  const selectedPosition = itemPositionMobile(isMobile ? 8 : 0, 5 - ((selectedIndex ?? index) - index) * (isDesktop ? 2 : 1), isSelected, isMobile, breakpoint)
  const imagePosition = itemPositionMobile(item.x, item.y, true, false, breakpoint)
  const imageBackgroundPosition = itemPositionMobile(item.x - 1, item.y - 1, true, isMobile, breakpoint)

  const selectedStyle = {
    height: `calc(${cssCellHeightMobile(isDesktop ? 2 : 1, false, breakpoint)} - ${cssUnit(isSelected && !isMobile ? 0 : 2, breakpoint)})`,
...
const itemPreviewPositionMobile = (x: number, y: number, isSelected = true, isMobile = false, b?: Breakpoint) => ({
  x: `calc(${cssCellWidth(x, true, b)} + ${cssUnit(isSelected || isMobile ? 0 : 2)})`,
  y: `calc(${cssCellHeightMobile(y, true, b)} + ${cssUnit(isSelected ? 0 : 1, b)})`,
})
