...
export const cellHeight2 = () =>
  (window.innerHeight - gridMarginY() * 2) / rows();
...
export const ArchiveRenderer = (props: ArchiveRendererProps) => {
  ...
  const [gridLayout, setGridLayout] = useState<GridLayoutData<ArchiveListItem>>(
    props.gridLayout
  );
  ...
  const selectedId = useMemo(
    () =>
      typeof selectedIndex === "number" && !isNaN(selectedIndex)
        ? gridLayout.items[selectedIndex]?.id || null
        : null,
  ...
  const indicatorProgress = useMemo(
    () =>
      doPreview && selectedIndex !== null
        ? selectedIndex / Math.max(props.archives.length - 1)
        : scrollProgress,
    [scrollProgress, doPreview, props.archives, selectedIndex]
  );
  ...
  const clearSelection = () => {
    ...
    scrollLimit.current = Math.max(
      gridLayout.matrixSize.height * cellHeight2() - cellHeight2() * 10,
      0
    );
  ...
  const onResize = useCallback(
    debounce(() => {
      ...
      const archiveUrls = layout.items.map(
        ({ extraData }) => extraData?.media.asset.url ?? ""
      );
      setColors(
        layout.items.map(({ extraData }) => extraData?.color ?? colors[0])
      );
      setCoverUrls(
        layout.items.map(
          (item, index) => item.extraData?.media.asset.url ?? archiveUrls[index]
        )
      );
      setGridLayout(layout);
      setBreakpoint(b);
    }, 500),
    [props.archives, colors, breakpointRef, doPreviewRef]
  );

  const changeActiveIndexOnScroll = useCallback(
    debounce(
      (n: 1 | -1) => {
        changeActiveIndex(n);
      },
  ...
  const onScroll = useCallback(
    ({ deltaY, ...e }: WheelEvent | { deltaY: number }) => {
      scrollYTarget.current = clamp(
        scrollYTarget.current + deltaY,
        0,
        scrollLimit.current
      );
  ...
  const onDragEnd = useCallback(
    (e: PointerEvent) => {
      ...
      if (doPreviewRef.current && dragging.current) {
        ...
        const n =
          deltaY === 0 ? 0 : ((deltaY / Math.abs(deltaY)) as 1 | -1 | 0);
        if (!isNaN(n) && n !== 0) {
          changeActiveIndex(n);
        }
      }
