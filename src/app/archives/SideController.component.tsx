...
export const SideController = ({
  ...
  return (
    <section className={cn(styles.sideController, [styles.hide, hide])}>
      <button
        className={cn(
          fontRepo.button.className,
          styles.controllerButton,
          styles.gridButton,
          [styles.active, !doPreview],
          [styles.hide, hide]
        )}
...
type IndicatorProps = Pick<
  SideControllerProps,
  "doPreview" | "hide" | "progress" | "breakpoint"
>;

const computeIndicatorCount = (breakpoint?: Breakpoint) => {
  if (typeof window === "undefined" || !window) return 15;
  return Math.round(
    (cellHeight(breakpoint) * (breakpoint === "mobile" ? 9 : 10)) /
      (breakpoint === "mobile" ? 13 : 17)
  );
};
...
const Indicator = ({
  ...
  useEffect(() => {
    if (!window) return;
    const onResize = () => {
      setCount(computeIndicatorCount(breakpoint));
    };
    ...
    () =>
      Array(count + spread)
        ...
        .map((_, index) => {
          ...
          const widthEased = easeInOutQuad(
            widthCoef,
            minPercent,
            1 - minPercent,
            1
    ...
    >
      {lineWidths.map((width, index) => (
        <div
          style={{ width }}
          key={"line" + index}
          className={l(styles.indicatorLine, hide)}
        ></div>
      ))}
    </div>
  );
